...
stockService.getStockByCompany =  async (company) => {
    const stock = await db.query('SELECT partwhse.companyID,partwhse.partID, part.partNum, part.description as partDescription, partclass.Description AS partClass,  warehouse.WareHouseCode, COALESCE(SUM(stock.qty),0) AS qty from partwhse LEFT JOIN part on part.ID = partwhse.partID left JOIN stock on stock.partID = partwhse.partID  and stock.warehouseID = partwhse.warehouseID left join warehouse on partwhse.warehouseID = warehouse.id left join partclass on partclass.ID = part.classID WHERE partwhse.companyID = ? group by warehousecode,partID', [company])
    return stock;
  };
